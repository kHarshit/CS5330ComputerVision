...
void createObject(const cv::Mat& rvec, const cv::Mat& tvec, const cv::Mat& camera_matrix, const cv::Mat& distortion_coefficients, const cv::Size& boardSize, cv::Mat& image)
{
    ...
    std::vector<cv::Point3f> person_standing ={
        ...
        cv::Point3f(4.0f,3.0f,-4.0f),   //body left end
        cv::Point3f(5.0f,3.0f,-4.0f),   // body right end
        cv::Point3f(4.5f,3.0f,-4.0f),   // neck start
        cv::Point3f(4.5f,3.0f,-4.25f),  // neck end
        cv::Point3f(4.5f,3.0f,-5.25f),  // head center
        cv::Point3f(5.75f,2.50f,-3.0f), //elbow right
        cv::Point3f(6.0f,1.80f,-4.25f),  //wrist right
        cv::Point3f(2.75f,2.40f,-2.75f),    //elbow left
        cv::Point3f(3.75f,3.7f,-1.8f)   //wrist left
    };
